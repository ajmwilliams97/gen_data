...
```{r}
data.table::fread("./02_activities/data/gwa.A2.fam")
```
The file contains 4000 samples.
(ii) Read the `.bim` file. How many SNPs does the dataset contain?

```{r}
data.table::fread("./02_activities/data/gwa.A2.bim")
```
...
```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/data/gwa.qc.A2_pruned --king-cutoff 0.0884 --out ./02_activities/data/gwa_qc.A2
```
...
```
```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated_pruned.A2 --pca 20 --out ./02_activities/data/gwa_unrel_PCA
```
...
```{bash,results='hide',message=FALSE, warning=FALSE}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --ci 0.95 --linear --covar ./02_activities/data/gwa_unrel_PCA.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.qc_assoc.A2
```

(ii) Create a Manhattan plot of the GWAS results.
```{r}
assoc <- data.table::fread("./02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear")

assoc_adjusted <- data.table::fread("./02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear.adjusted")
...
```{r}
#I wasn't sure if I needed to have the figures as png's or just embedded in the html file, so just did both. I wasn't sure how to do it more cleanly.
