...
#Because the p value is small, there is an association between having the C allele and the case phenotype.The positive value of the coefficient indicates that each additional C allele at this position increases the odds of the individual being a case.
...
p1=ggplot() +
  ...
  coord_cartesian(xlim = c(-0.5, 2.5), ylim = c(-2.7, 4.5)) +
  theme_minimal()
...
p2=ggplot() +
  ...
  coord_cartesian(xlim = c(-0.5, 1.5), ylim = c(-2.7, 4.5)) +
  theme_minimal()
...
The additive model fits better for this SNP. The AIC value for the additive model (10988) is slightly lower than the AIC value for the recessive model (11000). In addition, the additive model shows that the aa genotype has a case phenotype frequency less than 1.0 with only an n=14. When using recessive coding, the "0" includes heterozygotes which pushes that phenotypic frequency much higher than in the additive model. This could be inflating the influence of the aa genotype by combining it with the heterozygotes which have a higher phenotypic frequency when viewed through the additive model.
