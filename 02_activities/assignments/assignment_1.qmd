...
```{bash}
wc --lines ./02_activities/data/gwa.qc.A1.fam
```
...
wc --lines ./02_activities/data/gwa.qc.A1.bim
```
There are 101083 SNPs.
...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
      --extract ./02_activities/data/snplist.txt \
        --freq --out ./02_activities/data/gwa_qc_A1_freq
```
...
All of these allele frequencies are below 0.5, which means they represent the minor allele frequencies for these SNPs.

Therefore, the major allele frequencies are:

rs1861: 1-0.0539859=0.9460141, rs3813199:1-0.0569126=0.9430874, rs3128342:1-0.305121=0.694879, rs11804831: 1-0.154341=0.845659 

(ii) What are the minor allele frequencies (MAFs) for these four SNPs?
#From the frequencies calculated in the previous code, the minor allele frequencies are:
rs1861: 0.0539859, rs3813199:0.0569126, rs3128342:0.305121, rs11804831: 0.154341
...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
      --extract ./02_activities/data/snplist.txt \
        --hardy --out ./02_activities/data/gwa_qc_A1_hwe_snplist
```

```{r,message=FALSE, warning=FALSE}
hwesnplist <- fread("./02_activities/data/gwa_qc_A1_hwe_snplist.hardy")
head(hwesnplist)
```

The P-values for rs1861, rs3813199, rs3128342, and rs11804831 are:

rs1861: 0.274719, rs3813199: 1, rs3128342:0.330273, rs11804831: 0.113354
...
```{bash,message=FALSE, warning=FALSE}
printf "%s\n" rs1861 > ./02_activities/data/target.txt
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
      --extract ./02_activities/data/target.txt \
      --recode A \
      --out ./02_activities/data/geno.additive.target
```

```{r}
geno_subset <- fread("./02_activities/data/geno.additive.target.raw")
...
#Because the p value is small, there is an association between having the C allele and an increased expression of the phenotype.The positive value of the coefficient indicates that each additional C allele at this position increases the odds of the individual having a more intensely expressed phenotype for this trait.
...
p1=ggplot() +
  ...
  coord_cartesian(xlim = c(-0.5, 2.5), ylim = c(-2.7, 5.5)) +
  theme_minimal()
  
  

print(p1)
ggsave("./02_activities/data/p1_plotA1.png", plot = p1, width = 7, height = 5, dpi = 300)
```
...
p2=ggplot() +
  ...
  coord_cartesian(xlim = c(-0.5, 1.5), ylim = c(-2.7, 5.5)) +
  theme_minimal()

print(p2)
ggsave("./02_activities/data/p2_plotA1.png", plot = p2, width = 7, height = 5, dpi = 300)
```


(vii) Which model fits better? Justify your answer.
         
The additive model fits better for this SNP. The AIC value for the additive model (11277) is slightly lower than the AIC value for the recessive model (11292) which indicates a better fit. In addition, the additive model shows that the aa genotype has phenotypic values less than 1.0 with only an n=15. When using recessive coding, the "0" includes heterozygotes which pushes that phenotypic frequency much higher than in the additive model. This could be inflating the influence of the aa genotype by combining it with the heterozygotes which have a higher phenotypic frequency when viewed through the additive model. It also underestimates the influence of the Aa genotype which, when seen in the additive model, does have higher phenotypic values than the aa genotype. When observing the linear regression line, the recessive model indicates that having aa or Aa genotype is associated with a 0 phenotype value and a genotype of AA is associated with a ~0.5 phenotype value. In contrast, the additive model shows that heterozygotes have an increased risk compared to aa homozygotes and AA homozygotes have a phenotype value of approximately 2. This shows a gradual increase in risk compared to the recessive model that potentially underestimates the influence of the Aa genotype. 
